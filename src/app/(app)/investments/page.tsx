...
import { formatCurrency, INVESTMENT_TYPES, getInvestmentTypeLabel } from "@/lib/constants";

interface InvestmentTx { id: string; type: string; amount: number; units: number; pricePerUnit: number; note: string; date: string; }
interface Investment { id: string; name: string; type: string; costBasis: number; currentValue: number; units: number; note: string; transactions: InvestmentTx[]; }
...
export default function InvestmentsPage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    const res = await fetch("/api/investments", {
      ...
      body: JSON.stringify({ name: formName, type: formType, amount: formAmount, units: formUnits, pricePerUnit: formPrice, note: formNote, date: formDate }),
    });
    if (res.ok) { setShowAddModal(false); setFormName(""); setFormAmount(""); setFormUnits(""); setFormPrice(""); setFormNote(""); fetchInvestments(); }
  }
              ...
              <div key={inv.id} className="bg-[var(--card-bg)] rounded-xl p-5 shadow-[var(--shadow-card)] border border-[var(--card-border)]">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <span className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium bg-[var(--badge-muted-bg)] text-[var(--badge-muted-text)] mt-1">{getInvestmentTypeLabel(inv.type)}</span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-3">
                  <div>
                    <p className="text-[10px] text-[var(--text-tertiary)] uppercase">มูลค่า</p>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => openTxModal(inv, "value_update")} className="flex-1 py-2 bg-[var(--info-bg)] text-[var(--info-text)] rounded-lg text-[12px] font-medium">อัพเดทมูลค่า</button>
                </div>
              </div>
            );
          })}
        ...
        <div className="fixed inset-0 bg-[var(--modal-overlay)] flex items-center justify-center z-[200]" onClick={() => setShowAddModal(false)}>
          <div className="bg-[var(--modal-bg)] rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-[var(--shadow-lg)] border border-[var(--card-border)]" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleAdd}>
              ...
              <div className="mb-4">
                <label className="block text-[13px] font-medium mb-1.5 text-[var(--text-primary)]">จำนวนเงินลงทุน (บาท)</label>
      ...
      {showTxModal && txInv && (
        <div className="fixed inset-0 bg-[var(--modal-overlay)] flex items-center justify-center z-[200]" onClick={() => setShowTxModal(false)}>
          <div className="bg-[var(--modal-bg)] rounded-2xl p-6 w-full max-w-md shadow-[var(--shadow-lg)] border border-[var(--card-border)]" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[13px] text-[var(--text-secondary)] mb-5">{txInv.name} - มูลค่า {formatCurrency(txInv.currentValue)} บาท</p>
            <form onSubmit={handleTx}>
              <div className="mb-4">
                <label className="block text-[13px] font-medium mb-1.5 text-[var(--text-primary)]">
                  {txMode === "value_update" ? "มูลค่าปัจจุบัน (บาท)" : "จำนวนเงิน (บาท)"}
                </label>
